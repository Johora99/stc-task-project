...
import CompanyTypeSelect from "./SupportatCom";
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <Stack spacing={1}>
            {formData.offerings.map(o => (
              <FormControlLabel
                key={o.id}
                control={<Checkbox checked={o.checked} onChange={() => toggleOffering(o.id)} />}
                label={o.name}
              />
            ))}
          </Stack>
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <Stack direction="row" spacing={1}>
            {formData.images.map((img, index) => (
              <Box key={index} sx={{ width: 100, height: 100, border: "1px dashed #ccc", borderRadius: 2, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
                {img ? (
